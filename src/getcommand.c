...
static	FlowCom fcom[] = {
  ...
  { "Mark",		1, {0,0}, FALSE,	MARK		},
  { "Go",		1, {0,0}, FALSE,	GO			},
  { "Macro",	2, {0,0}, FALSE,	MACRO		},
  { "EndMacro",	0, {0,0}, FALSE,	ENDMACRO	},
  { "Expand",	1, {0,0}, FALSE,	EXPAND		},
  { "%",        0, {0,0}, FALSE,	COMMENT		},
...
static	FlowCom *code_to_command ( int code ) {
	int i;

	for ( i = 0; fcom[i].name != NULL; i++ ) {
		if ( fcom[i].command == code ) {
			return fcom + i;
		}
	}
		...
		if ( fcom[ i ].name == NULL ) {
			...
			if ( command == NULL ) {
				if ( findMacro ( token ) != NULL ) {
					command = code_to_command ( EXPAND );
					pPtr = line;
				}
			}
		}
